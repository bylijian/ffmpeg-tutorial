...
int main(int argc, char *argv[]) {
  ...
  if(aCodecCtx==NULL) {
    fprintf(stderr, "Unsupported audio codecContext!\n");
    return -1; // Codec not found
  }
  ...
  dev = SDL_OpenAudioDevice(NULL, 0, &wanted_spec, &have_spec, SDL_AUDIO_ALLOW_FORMAT_CHANGE);
	if (dev == 0) {
    fprintf(stderr,"Failed to open audio: %s", SDL_GetError());
		return -1;
	} else {
    if (have_spec.format != wanted_spec.format) { /* we let this one thing change. */
       fprintf(stderr,"We didn't get Float32 audio format.");
    }
